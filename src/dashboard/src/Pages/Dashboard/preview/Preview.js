    ...
    <Grid container direction="column">
      <div className={classes.preview}>
        ...
        <table>
          ...
          <tr valign="top">
            <td>
              {roomDoors.map((item, index) =>
                <div>{"to " + (roomDoors[index]?.toRoom === canvasRoom ? roomDoors[index]?.fromRoom : roomDoors[index]?.toRoom) + ' - ' + (roomDoors[index]?.doorIsLocked ? " Locked" : " Unlocked")}</div>)}
            </td>
            <td>
              {rooms[rooms.findIndex(item => item.id === activeRoom)]?.windows.map((item, index) =>
                <div>{"Window " + (index + 1) + ' - ' + (item.windowIsOpen ? " Open" : " Closed")}</div>)}
            </td>
            <td>
              {rooms[rooms.findIndex(item => item.id === activeRoom)]?.lights.map((item, index) =>
                <div>{"Light " + (index + 1) + ' - ' + (item.lightIsOn ? " On" : " Off")}</div>)}
            </td>
          </tr>
        </table>
      </div>
